...
Our data is sourced from the Open Data Toronto portal [@OpenDataToronto], specifically from the "Major Crime Indicators" dataset, which records reported crimes across Toronto. Compiled by the Toronto Police Service, the dataset includes details such as crime categories, locations, and times, enabling an analysis of crime trends and patterns.

By examining factors like crime type, neighborhood, and temporal distribution, this study aims to identify areas and times of higher crime occurrence. The findings can support targeted interventions and resource allocation. Following principles outlined in Telling Stories with Data [@tellingstories], this study prioritizes clarity and reproducibility to inform community safety strategies effectively.
...
@fig-modelresults1 shows the range of coefficient estimates for our model within the 90% probability interval. However, because the credibility intervals for several predictors are relatively narrow, particularly for premises types like "Transit," it is challenging to observe the trends clearly. To address this, @fig-modelresults2 was created with the x-axis limited to a range of -5 to 5. Combining @fig-modelresults1 and @fig-modelresults2, we observe statistical significance for several predictors, including "Transit," "Afternoon," and "Evening," as their credibility intervals do not cross zero.
...
Exploring time with finer granularity, such as hourly trends, could reveal patterns that are currently obscured. Additionally, combining behavioral data—such as crowd size or environmental conditions—with existing predictors may uncover new findings, such as the combined impact of transit activity and evening hours on crime risk.

## Conclusion

This study identifies the significant roles that premises type and time of day play in shaping the likelihood of violent crimes in Toronto. High-risk locations, such as transit hubs and outdoor spaces, and peak times, such as afternoons and evenings, highlight areas for focused interventions. These findings can inform practical safety measures, such as improved surveillance and better resource allocation during essential times and in high-risk areas. Although the analysis is limited by its broad categories, it demonstrates the value of understanding crime patterns through spatial and temporal dimensions, laying the groundwork for future research and actionable safety strategies.
...
The total budget for this survey is $100,000, allocated as follows:

- **Recruitment and Outreach ($25,000):**
  - Partnering with Toronto Police Services to contact victims via official channels.
  - Direct mailers and digital outreach (e.g., SMS, emails).

- **Incentives for Participation ($60,000):**
  - $20 gift card for completing the survey.
  - Estimated 3,000 participants.

- **Survey Design and Implementation ($10,000):**
  - Professional survey platform subscription.
  - Data cleaning and preprocessing costs.

- **Contingency Funds ($5,000):**
  - Unanticipated costs related to participant recruitment or data handling.

This allocation prioritizes participant incentives to ensure robust response rates while maintaining sufficient funding for recruitment and operational expenses.

## Survey Instrument

The questionnaire is designed to collect detailed information from victims of violent crimes. Attention checks are included to ensure data quality. The survey link: [Survey Form](https://forms.gle/d81J3S9HpPgJPUkM6).
...
### Informed Consent
- Participants will be informed about the purpose, risks, and benefits of the survey. Consent is mandatory.

### Confidentiality
- Responses will be anonymized. Data will be securely stored and used only for research purposes.

### Protection of Participants
- Sensitive topics will be handled by trained staff. Resources for support services will be provided.

### Ethical Approval
- The study protocol will undergo review by a Research Ethics Board (REB).

## Data Collection Procedures

### Mode of Data Collection
- **Primary Method**: Online surveys through a secure platform.
- **Alternative Methods**: Telephone interviews for participants with limited internet access.

### Timeline
- Data collection will be completed within two months of the survey launch.

### Contact Protocol
- Initial invitations via email or mail, followed by reminders to maximize participation.
...
#| tbl-cap: "Preview of the Simulated Crime Dataset"
#| echo: false
#| warning: false
#| message: false
...
t_simulated_data <- simulated_data[1:5, ] %>%
  t() %>%
  as.data.frame()

# Convert row names to a column for display
t_simulated_data <- tibble::rownames_to_column(t_simulated_data, var = "Variable")

# Display the transposed data with variables on the left
t_simulated_data %>%
  kable()
```
...
@fig-modelresults1 and @fig-modelresults2 visualize the 90% credible intervals for the predictors in the Bayesian logistic regression model. These plots provide intuitions into the uncertainty surrounding the estimated coefficients for each predictor.

@fig-modelresults1 displays the 90% credible intervals for all predictors without restrictions on the x-axis. This plot shows how the predictors such as `PREMISES_TYPE` and `TIME_OF_DAY` influence the likelihood of violent crimes. For example, `PREMISES_TYPETransit` has a high positive credible interval, indicating a significant increase in the likelihood of violent crime in transit areas. Conversely, `PREMISES_TYPECommercial` and `PREMISES_TYPEEducational` have negative credible intervals, indicating a lower likelihood of violent crimes in these premises types.

In @fig-modelresults2, the x-axis has been restricted to a range of -5 to 5 to better visualize predictors with smaller intervals. This refined view helps highlight subtle but meaningful differences between categories like `TIME_OF_DAYAfternoon` and `TIME_OF_DAYEvening`, which show a noticeable increase in the likelihood of violent crimes compared to the reference category (`TIME_OF_DAYEarly Morning`).
